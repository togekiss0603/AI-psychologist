...
def print_data_info(data_path):
    ...
    triples = []
    i = 0
    with open('D:\KBQA-study-main\data_model\medical.json', 'r', encoding='utf8') as f:
            ...
            for line in tqdm(f.readlines(), ncols=80):
                ...
                if 'cure_department' in data_json:
                    ...
                    if len(cure_department) == 2:
                        ...
                        self.rels_department.append([small,'belongs_to', big]) # 科室从属关系
                ...
                # 药品处理，前面是药企名称，()中是药物的名称
                if 'drug_detail' in data_json:
                    for det in data_json['drug_detail']:
                        det_spilt = det.split('(')
                        if len(det_spilt) == 2:
                            p, d = det_spilt  # p：药企的名称, d：药物的名称
                            d = d.rstrip(')')
                            if p.find(d) > 0:
                                p = p.rstrip(d)
                            self.producers.append(p)
                            self.drugs.append(d)
                            self.rels_drug_producer.append([p, 'production', d])  # 药企和药物的名称
                        else:
                            d = det_spilt[0]  # 默认是药物的名称
                            self.drugs.append(d)
...
if __name__ == '__main__':
    path = "D:\KBQA-study-main\data_model\medical.json"
